...
export const Battle = () => {
  ...
  const battles = useMemo(() => battle.battleData?.battles ?? [], [battle.battleData]);
  const hasBattles = battles.length > 0;

  const battleApplyItems = useMemo(
    () => battle.battleApplyList?.data?.battles ?? [],
    [battle.battleApplyList]
  );
  const hasBattleApplyList = battleApplyItems.length > 0;

  const periodOptions = battle.periodOptions;
  const analyzeOptions = battle.analyzeCategoryOptions;

  const rivalPercent = Math.round(battle.rivalPercent ?? 0);
  const myPercent = Math.round(battle.myPercent ?? 0);

  const getAnalyzePoint = (point: number | null | undefined) => {
    const value = Math.abs(point ?? 0);
    return battle.category === "ACTIVE_TIME" ? formatTime(Math.round(value)) : Math.round(value);
  };

  const rivalPoint = getAnalyzePoint(battle.rivalAnalyzePoint);
  const myPoint = getAnalyzePoint(battle.myAnalyzePoint);

  const diff = Math.round(battle.diff ?? 0);
                      ...
                      <S.TransitionBox>
                        <S.UpperHandTransition>
                          <S.UpperHandBar $width={battle.rivalPercent ?? 0} $isRival>
                            <S.PercentText $isRival>{rivalPercent}%</S.PercentText>
                          </S.UpperHandBar>

                          <S.UpperHandBar $width={battle.myPercent ?? 0} $isRival={false}>
                            <S.PercentText $isRival={false}>{myPercent}%</S.PercentText>
                          </S.UpperHandBar>
                        </S.UpperHandTransition>
                      ...
                      <S.AnalyzeBox>
                        <S.AnalyzeRow>
                          ...
                          <S.AnalyzeContent $width="100%">
                            <S.DataBox>
                              <div>
                                {rivalPoint} {battle.detailTextTranslate(battle.category)}
                              </div>
                              <S.AnalyzeBar $width={battle.rivalAnalyzeRate ?? 0} $isRival>
                                <S.AnalyzeLabel>
                                  {battle.isRivalHigher && diff > 0 && (
                                    <S.CompareDiff>+{diff}%</S.CompareDiff>
                                  )}
                                </S.AnalyzeLabel>
                              </S.AnalyzeBar>
                            </S.DataBox>
                            ...
                            <S.DataBox>
                              <div>
                                {myPoint} {battle.detailTextTranslate(battle.category)}
                              </div>
