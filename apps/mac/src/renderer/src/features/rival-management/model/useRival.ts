...
import {
  ...
  useMyRivalsQuery,
  useRivalListQuery,
  useRivalSignAllQuery,
  type MyRivalsResponse,
  type RivalApplyRequest,
  type RivalSignAllResponse,
  type RivalUsersResponse,
} from "@/entities/rival";
...
export const useRival = () => {
  ...
  const cancelRivalSignMutation = useMutation({
    ...
    onSettled: async () => {
      await queryClient.invalidateQueries({ queryKey: rivalQueryKeys.requests });
      await queryClient.invalidateQueries({ queryKey: rivalQueryKeys.myRivals });
    },
  });
